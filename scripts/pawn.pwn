...
new gPlayerStatus[MAX_PLAYERS][e_STATUS];
new Text:gRespawn;
new bool:gGameInProgress;

forward Restart();

public OnGameModeInit() {
    printf("\n----------------------------------");
    printf(" gungame is a gun game mode released as");
    printf(" an example mode for open.mp");
    printf("----------------------------------\n");
}
...
public EndRound() {
    ...
    new highest[3] = {INVALID_PLAYER_ID, INVALID_PLAYER_ID, INVALID_PLAYER_ID};
    ...
    format(string, sizeof(string),
        ...
        GetPlayerScore(highest[0]), pName[0],
        GetPlayerScore(highest[1]), pName[1],
        GetPlayerScore(highest[2]), pName[2]
    );
